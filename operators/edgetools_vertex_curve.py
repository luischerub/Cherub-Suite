...
from .edgetools import interpolate
...
def curve_hermite(bm, selected, vert_path, tension, space_evenly):
    ...
    for index, segment in enumerate(segments):
        ...
        if not space_evenly:
            interpolate.map_segment_onto_spline(segment, spline_points)
        else:
            total_spline.extend(spline_points)

    if space_evenly:
        interpolate.map_segment_onto_spline(vert_path, total_spline)
...
def circle_3_points(bm, selected, vert_path, tension, space_evenly):
    ...
    if tension == -1.0:
        ...
        interpolate.map_segment_onto_spline(vert_path, positions)
        return 0, ""
...
class CHERUB_OT_VertexCurve(bpy.types.Operator):
    bl_idname = "mesh.cherub_vertex_curve"
